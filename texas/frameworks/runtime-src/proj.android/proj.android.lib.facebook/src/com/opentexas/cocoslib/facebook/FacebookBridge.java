...
public class FacebookBridge {
    ...
    public static void statisticsFBADPay(final String num, final String code) {
        final FacebookPlugin plugin = getFacebookLoginPlugin();
        if (plugin != null) {
            Cocos2dxActivityUtil.runOnUIThreadDelay(new Runnable() {
                @Override
                public void run() {
                    Log.d(TAG, "plugin statisticsFBADPay begin");
                    plugin.statisticsFBADPay(num, code);
                    Log.d(TAG, "plugin statisticsFBADPay end");
                }
            }, 50);
        }
    }
