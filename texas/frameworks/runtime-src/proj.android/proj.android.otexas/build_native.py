...
jnibuildRoot = runtimeRoot + "/jnibuild"
runtimexRoot = os.path.split(runtimeRoot)[0]
runtimeRoot = runtimexRoot
frameworksRoot = os.path.split(runtimexRoot)[0]
...
def buildNative(isRelease):
    ...
    print "====> Moveing *.so to projLibs\n"
    libsDir = joinDir(projectRoot, "libs", "armeabi")
    jniLibsDir = joinDir(jnibuildRoot, "libs", "armeabi")
    if os.path.exists(libsDir):
        shutil.rmtree(libsDir)
    shutil.copytree(jniLibsDir, libsDir)
    print "====> Build Done\n"
    '''
...
def cleanNative():
    ...
    objDir = joinDir(jnibuildRoot, "obj")
...
if __name__ == "__main__":
    ...
    jniDir = joinDir(jnibuildRoot, "jni")
    #jniDir = joinDir(projectRoot, "libcocos2dx", "jni")
    if False == os.path.exists(jniDir):
        print "Error: %s is not a project directory" %(jnibuildRoot)
        sys.exit(-2)
