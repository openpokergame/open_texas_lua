...
public class ScreenShotUtil {
    ...
    public static String saveScreenShot(String fileName) {
        ...
        Bitmap bmp = takeScreenShot(GameMain.getContext());
    ...
    public static void screenShot(int methodId, int x, int y, int w, int h) {
        ...
        if (w == 0 || h == 0) {
            ...
            GameMain.getContext().getWindowManager().getDefaultDisplay().getMetrics(dm);
            w = dm.widthPixels;
            h = dm.heightPixels;
        }
        ...
        try {
            MediaStore.Images.Media.insertImage(GameMain.getContext().getContentResolver(), file.getAbsolutePath(), fileName, null);
        } catch (FileNotFoundException e) {
            Cocos2dxActivityUtil.runOnGLThread(new Runnable() {
                @Override
                public void run() {
        ...
        GameMain.getContext().sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://" + file.getAbsolutePath())));
        Cocos2dxActivityUtil.runOnGLThread(new Runnable() {
            @Override
            public void run() {
    ...
    public static void updateMediaStore(String filePath, String fileName, int methodId) {
        ...
        try {
            MediaStore.Images.Media.insertImage(GameMain.getContext().getContentResolver(), filePath, fileName, null);
        } catch (FileNotFoundException e) {
            Cocos2dxActivityUtil.runOnGLThread(new Runnable() {
                @Override
                public void run() {
        ...
        GameMain.getContext().sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://" + filePath)));
        Cocos2dxActivityUtil.runOnGLThread(new Runnable() {
            @Override
            public void run() {
