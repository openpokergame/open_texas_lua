...
function EditBoxManager:ctor()
	self.editboxList_ = {}
	self.onEditBoxTouchEnabledId_ = sa.EventCenter:addEventListener(tx.eventNames.ENABLED_EDITBOX_TOUCH, handler(self, self.onEditBoxTouchEnabled_))
    self.onEditBoxTouchDisenabledId_ = sa.EventCenter:addEventListener(tx.eventNames.DISENABLED_EDITBOX_TOUCH, handler(self, self.onEditBoxTouchDisenabled_))
end

--添加需要管理的输入框
--count默认为0，但有2种情况需要手动计数，1.当打开弹窗没有输入框，但是切换tab会有输入框 2.弹窗不是通过PopupManager添加到界面的
