...
function SocketBase:ctor(name, protocol)
    ...
    self.socket_ = sa.SocketService.new(name, protocol)
    self.socket_:addEventListener(sa.SocketService.EVT_PACKET_RECEIVED, handler(self, self.onPacketReceived))
    self.socket_:addEventListener(sa.SocketService.EVT_CONN_SUCCESS, handler(self, self.onConnected))
    self.socket_:addEventListener(sa.SocketService.EVT_CONN_FAIL, handler(self, self.onConnectFailure))
    self.socket_:addEventListener(sa.SocketService.EVT_ERROR, handler(self, self.onError))
    self.socket_:addEventListener(sa.SocketService.EVT_CLOSED, handler(self, self.onClosed))
    self.socket_:addEventListener(sa.SocketService.EVT_CLOSE, handler(self, self.onClose))
    ...
    self.heartBeatSchedulerPool_ = sa.SchedulerPool.new()

    self.logger_ = sa.Logger.new(self.name_)
end
...
function SocketBase:setExtendProtocol(protocol)
    self.socket_:setExtendProtocol(protocol)
end
...
function SocketBase:connect(ip, port, retryConnectWhenFailure)
    ...
    self.ip_ = ip
    self.port_ = port
    self.logger_:debugf("x connect to %s:%s", self.ip_, self.port_)
...
function SocketBase:onConnected(evt)
    ...
    self.logger_:debug("SocketBase connected")
...
function SocketBase:scheduleHeartBeat(command, interval, timeout)
    ...
    -- 立马启动 server开启超时一分钟断开  否则太早切后台连接不断开 
    self:onHeartBeat_()
end
...
function SocketBase:onHeartBeat_()
    ...
    if heartBeatPack then
        self.heartBeatPackSendTime_ = sa.getTime()
        self:send(heartBeatPack,true)
...
function SocketBase:onHeartBeatReceived_()
    local delaySeconds = sa.getTime() - self.heartBeatPackSendTime_
