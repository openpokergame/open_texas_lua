...
function DailyTasksEventHandler:addEvents()
    ...
    self:addEvent(tx.eventNames.HALL_LOGIN_SUCC, handler(self, self.onLogin_))
    self:addEvent(tx.eventNames.HALL_LOGOUT_SUCC, handler(self, self.onLogout_))
...
function DailyTasksEventHandler:addEvent(evt, func)
    sa.EventCenter:addEventListener(evt, func)
end
