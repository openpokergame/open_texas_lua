...
function AchievementRewardPopup:setupView()
    ...
    self.shareBtn_ = cc.ui.UIPushButton.new({normal = "#common_btn_blue_normal.png", pressed = "#common_btn_blue_pressed.png", disabled = "#common_btn_disabled.png"},{scale9 = true})
        :pos(0, -90)
        :setButtonSize(174, 55)
        :setButtonLabel(ui.newTTFLabel({text = sa.LangUtil.getText("COMMON", "SHARE"), color = TEXT_COLOR, size = 22, align = ui.TEXT_ALIGN_CENTER}))
        :onButtonClicked(handler(self, self.onShareClicked_))
        :addTo(self)
end

function AchievementRewardPopup:onShareClicked_()
    local feedData = clone(sa.LangUtil.getText("FEED", "ACHIEVEMENT_REWARD"))
    feedData.name = sa.LangUtil.formatString(feedData.name, self.achieve.name, self.achieve.reward)
...
function AchievementRewardPopup:showPanel()
    tx.PopupManager:addPopup(self)
    return self
end

function AchievementRewardPopup:hidePanel()
