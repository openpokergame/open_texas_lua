...
local logger = sa.Logger.new("XinGePushAndroid")
...
function XinGePushAndroid:getPushData()
...
function XinGePushAndroid:call_(javaMethodName, javaParams, javaMethodSig)
    if device.platform == "android" then
        local ok, ret = luaj.callStaticMethod("com/opentexas/cocoslib/xinge/XinGePushBridge", javaMethodName, javaParams, javaMethodSig)
        if not ok then
            if ret == -1 then
