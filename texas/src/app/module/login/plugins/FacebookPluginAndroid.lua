...
local logger = sa.Logger.new("FacebookPluginAndroid")
...
function FacebookPluginAndroid:bindGuest(callback)
    self:login(callback)
end
...
function FacebookPluginAndroid:shareFeed(params, callback)
    ...
    --     -- end
        ...
        if params.link then
            params.link = params.link .. "&caption=" .. params.caption .. "&name=" .. params.name
        end
...
function FacebookPluginAndroid:ShareBySystem(params)
    ...
    self.shareData.link = sa.LangUtil.getText("FEED", "SHARE_LINK")
    self:call_("ShareBySystem", {json.encode(self.shareData)}, "(Ljava/lang/String;)V")
end

--更多邀请
function FacebookPluginAndroid:moreInvite(params)
...
function FacebookPluginAndroid:onImageLoadComplete_()
    ...
    local path = tx.ImageLoader.cacheConfig_[tx.ImageLoader.CACHE_TYPE_SHARE].path
    if path then
...
function FacebookPluginAndroid:onShareFeedResult_(result)
    ...
    sa.EventCenter:dispatchEvent({name="SHARE_SDK_SUCCESS"})
end
...
-- FB广告统计支付
function FacebookPluginAndroid:statisticsFBADPay(num,code)
    self:call_("statisticsFBADPay", {num.."",code}, "(Ljava/lang/String;Ljava/lang/String;)V")
end

function FacebookPluginAndroid:call_(javaMethodName, javaParams, javaMethodSig)
    if device.platform == "android" then
        local ok, ret = luaj.callStaticMethod("com/opentexas/cocoslib/facebook/FacebookBridge", javaMethodName, javaParams, javaMethodSig)
        if not ok then
            if ret == -1 then
