local FAQListItem = class("FAQListItem", sa.ui.ListItem)

FAQListItem.WIDTH = 0
FAQListItem.HEIGHT = 96

FAQListItem.ANSWER_SIZE = 26
FAQListItem.ANSWER_COLOR = styles.FONT_COLOR.CONTENT_TEXT

local itemPadding = 6 --每个元素之间间隔 列表元素 高66 实际背景框高 56 上下间隔为5 2个元素之间的间隔就为10

local bottomPanelMinHeight_ = 1 --底部面板最小高度
...
function FAQListItem:ctor()
    ...
    --触摸层
    self.touchBtn_ = cc.ui.UIPushButton.new("#common/transparent.png", {scale9 = true})
    ...
    --底部背景，相当于底板
    self.bottomBackground_ = display.newScale9Sprite("#dialogs/help/question_content_bg.png")
    ...
    local bg = display.newScale9Sprite(
        "#dialogs/help/question_title_bg.png",
        0, 0,
    ...
    self.foldIcon_ = display.newSprite("#dialogs/help/arrow.png", FAQListItem.WIDTH * -0.5 + foldIconMarginLeft, -3):addTo(topPanel)
    self.foldIcon_:rotation(-90)

    --标题
    self.titleMarginLeft = foldIconMarginLeft + self.foldIcon_:getContentSize().width + 12
    self.title_ = ui.newTTFLabel({size=28, align=ui.TEXT_ALIGN_LEFT, dimensions = cc.size(self.contentWidth - 90, 0)})
    ...
    self.answerLabel = ui.newTTFLabel({
            ...
            dimensions = cc.size(self.contentWidth - 90, 0)})
...
function FAQListItem:onEnterFrame_()
    ...
    if self.heightExtra_ == dest then
        ...
        self.heightExtra_ = self.heightExtra_ + direction * math.max(1, math.abs(self.heightExtra_ - dest) * 0.3)
        if direction > 0 and self.heightExtra_ > dest or direction < 0 and self.heightExtra_ < dest then
            self.heightExtra_ = dest
        end
    end
    self.foldIcon_:rotation(-90+90 * (self.heightExtra_ / (bottomHeight)))
