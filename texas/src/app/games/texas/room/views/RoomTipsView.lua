...
function RoomTipsView:ctor()
    self.TIPS = sa.LangUtil.getText("ROOM", "TIPS")
    self.background_ = display.newScale9Sprite("#texas/room/tips_background.png", 0, 0, cc.size(RoomTipsView.WIDTH, 72)):addTo(self)
    self.schedulerPool_ = sa.SchedulerPool.new()
    self.isPlaying_ = false
    self.indexArr_ = self:randomIndexArr_(#self.TIPS)
    ...
    self.label_ = ui.newTTFLabel({size=24, color=TEXT_COLOR,
            text=self.TIPS[self.currentIndex_],
...
function RoomTipsView:playNext_()
    if self.isPlaying_ then
        ...
        if self.currentIndex_ > #self.TIPS then
            self.currentIndex_ = 1
        end
