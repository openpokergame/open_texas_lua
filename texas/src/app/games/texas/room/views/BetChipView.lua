...
function BetChipView:rotate(positionId)
    if self.betChipData_ then
        -- 调整
        local coord = BP[positionId]
        if positionId==5 and self.manager_ and self.manager_.model and self.manager_.model:isSelfInSeat() then
            coord = BP[10]
        end
        local curOffX = CHIPS_OFFX
        local curOffY = CHIPS_OFFY
        if positionId~=2 and positionId~=3 then
            curOffX = (-curOffX)
        end
        for i, chipData in ipairs(self.betChipData_) do
            chipData:getSprite():pos(coord.x + curOffX, coord.y + curOffY + (i - 1) * GAP_WITH_CHIPS)
        end
        return self
    end
end
    ...
    if self.betChipData_ then
        ...
        for i, chipData in ipairs(self.betChipData_) do
            ...
            if sp:getParent() then
                sp:pos(coord.x + curOffX, coord.y + curOffY + (i - 1) * GAP_WITH_CHIPS):opacity(255)
            else
                sp:pos(coord.x + curOffX, coord.y + curOffY + (i - 1) * GAP_WITH_CHIPS):opacity(255):addTo(self.parent_)
            end
        end
...
function BetChipView:resetChipStack(betChips)
    ...
    if self.betTotalChips_ > 0 then
        ...
        self.betChipData_ = self.manager_:getChipData(self.betTotalChips_, true)
...
function BetChipView:moveFromSeat(betNeedChips, betTotalChips)
    if betNeedChips > 0 and betTotalChips > 0 then
        ...
        -- 获取本次筹码数据
        self.movingChipData_ = self.manager_:getChipData(betNeedChips, true)
        self.betChipData_ = self.manager_:getChipData(betTotalChips, true)
            ...
            if i < #self.movingChipData_ then
                transition.execute(
                    ...
                    cc.MoveTo:create(
                        ...
                        cc.p(coord.x + curOffX, coord.y + curOffY + (i + lastBetChipNum - 1) * GAP_WITH_CHIPS)
                    ), 
