...
function CommonPopupTabBar:ctor(args, txtSize)
    ...
    if #args.btnText==1 then
        self.bgWidth_,item_width = 160,160
    end
    local item_offset_y = -10
    local item_offset_x = 1


    display.newScale9Sprite(args.bg or "#common/pop_tab_bg.png", 0, item_offset_y, cc.size(self.bgWidth_, self.bgHeight_), cc.rect(33, 23, 1, 1))
        :addTo(container)
    ...
    local itemFirstPressedbg = nil
    local itemFirstSelectedbg = nil
    if #args.btnText==1 then
        itemFirstPressedbg = display.newScale9Sprite(
                args.edgeBg or "#common/pop_tab_item_pressed.png",
                -item_width * 0.5 + item_offset_x,
                item_offset_y,
                cc.size(firstBgW*0.5+20, firstBgH),
                args.edgeRect or cc.rect(25, 22, 1, 1)
            ):addTo(container, 2)
            :hide()
        itemFirstPressedbg:pos(-item_width * 0.5 + item_offset_x + 48,item_offset_y)
        local right_ = display.newScale9Sprite(
                args.edgeBg or "#common/pop_tab_item_pressed.png",
                -item_width * 0.5 + item_offset_x,
                item_offset_y,
                cc.size(firstBgW*0.5+20, firstBgH),
                args.edgeRect or cc.rect(25, 22, 1, 1)
            ):addTo(itemFirstPressedbg)
        right_:setScaleX(-1)
        right_:pos(firstBgW*0.5+30,firstBgH*0.5)

        itemFirstSelectedbg = display.newScale9Sprite(
                args.edgeSelectedBg or "#common/pop_tab_item_selected.png",
                -item_width * 0.5 + item_offset_x,
                item_offset_y,
                cc.size(firstBgW*0.5+20, firstBgH),
                args.edgeRect or cc.rect(50, 22, 1, 1)
            ):addTo(container, 3):hide()
        itemFirstSelectedbg:pos(-item_width * 0.5 + item_offset_x + 48,item_offset_y)
        local right_ = display.newScale9Sprite(
                args.edgeSelectedBg or "#common/pop_tab_item_selected.png",
                -item_width * 0.5 + item_offset_x,
                item_offset_y,
    ...
    if #args.btnText == 4 then
        ...
        local itemPressedBg2_ = display.newScale9Sprite(
                args.middleBg or "#common/pop_tab_item_middle_pressed.png",
                -middleBgW * 0.5,
                item_offset_y,
        ...
        local itemSelectedbg2_ = display.newScale9Sprite(
                args.middleSelectedBg or "#common/pop_tab_item_middle_selected.png",
                -middleBgW * 0.5,
                item_offset_y,
                cc.size(middleBgW, middleBgH),
                args.middleRect or cc.rect(10, 22, 1, 1)
        ...
        display.newSprite("#common/pop_tab_item_divide.png"):addTo(container):pos(-item_width, item_offset_y)
        display.newSprite("#common/pop_tab_item_divide.png"):addTo(container):pos(0, item_offset_y)
        display.newSprite("#common/pop_tab_item_divide.png"):addTo(container):pos(item_width, item_offset_y)
    ...
    if #args.btnText == 3 then
        ...
        display.newSprite("#common/pop_tab_item_divide.png"):addTo(container):pos(-item_width * 0.5, item_offset_y)
        display.newSprite("#common/pop_tab_item_divide.png"):addTo(container):pos(item_width * 0.5, item_offset_y)
    ...
    for i = 1, #args.btnText do
        if args.iconTexture then
            ...
            self.btnIconsBg_[i] = display.newSprite("#common/popup_tab_bar_icon_selected.png"):pos(args.iconOffsetX, 0)
        end

        self.subBtns_[i] = cc.ui.UIPushButton.new({normal = "#common/transparent.png"}, {scale9 = true})
...
function CommonPopupTabBar:gotoTab(btnId)
    ...
    for i, v in ipairs(self.subBtns_) do
        ...
        if i == btnId then
            ...
            if icon then
                icon:setSpriteFrame(string.gsub(self.iconTexture_[i][1], "#", ""))
            end

            if iconBg then
                iconBg:setSpriteFrame("common/popup_tab_bar_icon_selected.png")
            end
            ...
            if icon then
                icon:setSpriteFrame(string.gsub(self.iconTexture_[i][2], "#", ""))
            end
            if iconBg then
                iconBg:setSpriteFrame("common/popup_tab_bar_icon_unselected.png")
            end
        end
    end
