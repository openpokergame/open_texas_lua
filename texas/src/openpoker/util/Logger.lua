...
Logger.LEVEL_FATAL = 1

Logger.LEVEL_TAGS = {"FATAL", "ERROR", "WARN ", "INFO ", "DEBUG"}
...
function Logger:fatal(...)
    return self.isEnabled_ and self:log_(true, Logger.LEVEL_FATAL, self.name_, "%s", self:concatParams(...)) or self
end

function Logger:fatalf(fmt, ...)
    return self.isEnabled_ and self:log_(true, Logger.LEVEL_FATAL, self.name_, fmt, ...) or self
end
...
function Logger:log_(stackTrace, level, tag, fmt, ...)
    if CF_DEBUG >= level then
        ...
        for i, v in ipairs(Logger.outputs) do
            ...
            if writeLog then
                writeLog(string.format("[%s][%s][%s] " .. fmt, Logger.LEVEL_TAGS[level], tag, debug.getinfo(3).name or "main", ...))
            end
