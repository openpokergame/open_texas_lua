...
local lime = require 'openpoker.util.Lime'
...
function functions.cacheFile(url, callback, dirName)
    ...
    if functions.mkdir(dirPath) then
        if io.exists(filePath) then
            ...
            sa.HttpService.GET_URL(url, {}, function(data)
...
--转换成百分比数
function functions.transformToPercent(cur, total)
    if total == 0 then
        return "0%"
    end
...
function functions.transformDataToGroup(data, rowNum)
    local ret = {}
    if data then
        for i = 1, #data do
            local m = math.floor(i / rowNum) + 1
            local n = i % rowNum
            if n == 0 then
                n = rowNum
                m = m - 1
...
function functions.sortFriendData(data)
    for _, v in ipairs(data) do
        if v.serverGameid and v.serverTid and tonumber(v.serverGameid)>0 and tonumber(v.serverTid)>0 then
            v.isTrack = 1
        else
            v.isTrack = 0
        end
    ...
    local function sort_(a, b)
        local r
        local a_online = tonumber(a.isOnline) or 0 --是否在线
        local b_online = tonumber(b.isOnline) or 0

        local a_isTrack = tonumber(a.isTrack) or 0 --是否可追踪
        local b_isTrack = tonumber(b.isTrack) or 0

        local a_isFb = tonumber(a.isFb) or 0 --是否FB登录
        local b_isFb = tonumber(b.isFb) or 0

        local a_isRecall = tonumber(a.isRecall) or 0 --是否需要召回
        local b_isRecall = tonumber(b.isRecall) or 0
...
--[[
local tmp = sa.cloneNode(self.inviteNode_, cc.size(270, 47), 270*0.5, 47*0.5);
...
function functions.GaussianBlurNode(node, scaleFactor, pixelformat, filePath)
    ...
    local cnode = cc.Sprite:createWithTexture(tex);
    cnode:setFlippedY(true)
    sa.DisplayUtil.setGaussian(cnode, {sampleNum=8, radius=4})
...
function functions.getBlurBgPath(filename)
    if not functions.blurBgPath_ then
        ...
        sa.mkdir(functions.blurBgPath_)
    end
...
function functions.createBlurBg(node, filename, scaleFactor)
    ...
    if not io.exists(fullPath) then
        local cnode = functions.GaussianBlurNode(tx.runningScene, scaleFactor, nil, filePath):scale(1 / scaleFactor)
        if not io.exists(fullPath) then
            return cnode
        end
    end
...
function functions.testDraw(contains)
    local children = contains:getChildren()
    ...
    for i, child in ipairs(children) do
        ...
        local testNode = display.newScale9Sprite("#common/common_red_btn_up.png", px, py, cc.size(size.width, size.height))
            :addTo(parent)
