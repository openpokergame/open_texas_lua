...
function ScrollView:setScrollContent(content)
    ...
    local useContentSize
    if self.direction_ == ScrollView.DIRECTION_VERTICAL then
        useContentSize = contentSize.height > 0
    ...
    elseif self.direction_ == ScrollView.DIRECTION_HORIZONTAL then
        ...
        local casRectDW = casRect.width or 0
        local viewDW = self.viewRect_.width or 0
        useContentSize = contentSize.width > 0
...
function ScrollView:moveDistance(distance)
    -- 关闭帧频回调
    self.scrollContent_:unscheduleUpdate()

    local dragTime = 0.6

    self.speed_ = distance/(dragTime * 1000) * ScrollView.SPEED_ADJUST_RATE
    self.speed_ = math.abs(self.speed_)
...
function ScrollView:getContentPlace()
    return self.srcContentPlace_
end

-- -- 不能添加到九宫格图片中，否则会出现点击区域一直被触发
function ScrollView:addTo(parent,noCheck)
    if noCheck == true then
        cc.Node.addTo(self,parent)
        return self
    end
    local isScale9 = false
    local scale9Node = nil
    -- 找到最上层为scale9的节点
    local fun = nil
    fun = function(pa)
        if pa then
            if pa.setSpriteFrame and pa.getCapInsets then
                isScale9 = true
                scale9Node = pa
                -- 显示
                pa.setVisible = function(obj,value)
                    if value==false then
                        self:setVisible(value,true)
                        cc.Node.setVisible(obj,value)
                    else
                        if self.visibleValue__~=false then
                ...
                -- 删除
                pa.removeFromParent = function(obj,value)
                    cc.Node.removeFromParent(obj,value)
                    cc.Node.removeFromParent(self,value)
                end
            end
            fun(pa:getParent())
        end
    ...
    fun(parent)
    if isScale9==true and scale9Node then
        -- 覆盖
        self.setVisible = function(obj,value,fromParent)
            if not fromParent then
                obj.visibleValue__ = value
            end
            cc.Node.setVisible(obj,value)
            return obj
        end

        local finalParent = scale9Node:getParent()
        if finalParent then
            local tempWorldPt = parent:convertToWorldSpace(cc.p(self:getPosition()))
            local tempLocalPt = finalParent:convertToNodeSpace(tempWorldPt)
            cc.Node.addTo(self,finalParent,scale9Node:getLocalZOrder()+1)
            self:pos(tempLocalPt.x,tempLocalPt.y)
        end
    else
        cc.Node.addTo(self,parent)
    end
