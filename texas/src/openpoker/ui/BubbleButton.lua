...
local Default_ResID = "#common/transparent.png"
function BubbleButton.createCommonBtn(params)
    ...
    if strokeColor and icon then
        strokeSpr = sa.createStroke(icon, strokeWidth, strokeColor, strokeOpacity, strokeStep)
        strokeSpr:hide()
    end
        ...
        if iconOverResId ~= iconNormalResId then
            if icon then
                if string.byte(iconNormalResId) == 35 then
                    icon:setSpriteFrame(string.sub(iconNormalResId, 2))         
                else
                    icon:setSpriteFrame(iconNormalResId)
                end
            end
        end
    ...
    btn:onButtonPressed(function()
        ...
        if iconOverResId ~= iconNormalResId then
            if string.byte(iconOverResId) == 35 then
                icon:setSpriteFrame(string.sub(iconOverResId, 2))            
            else
                icon:setSpriteFrame(iconOverResId)
            end
        end
    ...
    if imgParams and imgParams.url then
        ...
        imageLoaderId = tx.ImageLoader:nextLoaderId()
        tx.ImageLoader:loadAndCacheImage(imageLoaderId, imgParams.url, function(success, sprite)
    ...
    node.onCleanup = function()
        if imageLoaderId then
            tx.ImageLoader:cancelJobByLoaderId(imageLoaderId)
        end
    end
    node.icon = icon
    node.txt = txt
