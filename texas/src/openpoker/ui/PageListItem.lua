...
function PageListItem:setData(data,startIndex,itemClass)
    ...
    for i=1,count do
        ...
        if not item then
            item = itemClass.new()
                :setIndex(startIndex+i-1)
                :setOwner(self.pageView_)
...
function PageListItem:lazyCreateContent()
    local item = nil
    for i=1,#self.itemList_ do
        item = self.itemList_[i]
        if item and item.lazyCreateContent then
            item:lazyCreateContent()
        end
    end
end

function PageListItem:onItemDeactived()
    local item = nil
    for i=1,#self.itemList_ do
        item = self.itemList_[i]
        if item and item.onItemDeactived then
