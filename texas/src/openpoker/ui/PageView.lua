...
function PageView:ctor(params, itemClass, pageParams)
    ...
    if pageParams~=nil then
        ...
        self.btnNormal_ = pageParams.btnNormal or "#common/page_btn_normal.png"
        self.btnSelect_ = pageParams.btnSelect or "#common/page_btn_select.png"
    end
    -- 滚动容器
...
-- 设置翻页按钮的偏移量
function PageView:setPagePointOffset(x, y)
    if self.btnContain_ then
        x = x or 0
        y = y or 0
        local xx,yy=self.btnContain_:getPosition()
        self.btnContain_:pos(xx+x, yy+y)
    end
end
-- 滚动容器变化时需要更新
function PageView:update()
    ...
    for i=1,length do
        ...
        if not item then
            ...
            self.btns_[i] = item
            sa.TouchHelper.new(item, function(target,evtName)
                if evtName==sa.TouchHelper.CLICK then
                    if self.stopIndex_~=i then
                        self:gotoPage(i)
                    end
                -- elseif evtName==sa.TouchHelper.TOUCH_BEGIN then
                -- elseif evtName==sa.TouchHelper.TOUCH_MOVE then
                end
            end)
        end
