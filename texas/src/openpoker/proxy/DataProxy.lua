...
-- 清理一个data，保留handler  removeEvent删除了
function DataProxy:clearData(key,removeEvent)
    if self:hasData(key) then
        ...
        if removeEvent then
            if keyHandler[key] then
                for _, handler in localPairs(keyHandler[key]) do
                    keyHandler[key][_] = nil
                end
            end
            if propertyHandler[key] then
                for _, handlerTable in localPairs(propertyHandler[key]) do
                    for __, handler in localPairs(handlerTable) do
                        propertyHandler[key][_][__] = nil
                    end
                end
            end
        end
