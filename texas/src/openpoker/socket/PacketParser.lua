...
function PacketParser:ctor(protocol, socketName)
    self.config_ = protocol.RECE
    self.protocol_ = protocol
    self.logger_ = sa.Logger.new(socketName .. ".PacketParser"):enabled(true)
end

function PacketParser:setExtendProtocol(protocol)
    if protocol then
        self.extConfig_ = protocol.RECE
    else
        self.extConfig_ = nil
    end
end
...
local function verifyHeadAndGetBodyLenAndCmd(buf)
    ...
    if buf:readStringBytes(2) == "SA" then
        buf:setPos(9)
        cmd = buf:readUInt()
        buf:setPos(1)
        len = buf:readUInt()
...
function PacketParser:parsePacket_(buf)
    ...
    self.logger_:debugf("ReceivePacket time is ==>"..TimeUtil:getTimeStampString(os.time(),nil,true))
    local ret = {}
    local cmd = buf:setPos(9):readUInt()
    local GAMEID = buf:readUShort()
    -- local ver = buf:readByte()
    -- local type_ = buf:readByte()
    local config = self.config_[cmd]
    if config==nil and self.extConfig_ then
        config = self.extConfig_[cmd]
    end
    if config ~= nil then
