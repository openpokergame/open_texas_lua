...
function UpdateController:ctor()
    ...
    -- 默认语言
    _G.autoSearchPath = cc.FileUtils:getInstance():getSearchPaths()
    cc.FileUtils:getInstance():addSearchPath("res/"..appconfig.LANG.."/")  --语言优先
...
function UpdateController:startUpdate()
    ...
    self.view_:showLoadingStrAnim(upd.lang.getText("UPDATE", "CHECKING_VERSION"))
        ...
        upd.http.POST_URL(appconfig.VERSION_CHECK_URL, 
            ...
            function (data)
                ...
                if retData then
                    ...
                    -- 更新动态获取
                    local hotfix = retData.hotfix
                    if hotfix and hotfix~="" then
                        upd.conf.SERVER_FILE_URL_FMT = hotfix.."%s?dev="..device.platform .."&%s"
                    end
...
function UpdateController:startHotUpdate()
    ...
    self.view_:showLoadingStrAnim(upd.lang.getText("UPDATE", "CHECKING_RES_UPDATE"))
...
function UpdateController:endUpdate_(fileList)
    ...
    --导出到全局命名空间
    SA_UPDATE = {}
    SA_UPDATE.VERSION = version
    SA_UPDATE.FACEBOOK_BONUS = self.FACEBOOK_BONUS
    SA_UPDATE.FEEDBACK_URL = self.feedBackUrl
    SA_UPDATE.STAGE_FILE_LIST = clone(fileList.stage)
    ...
    loaderFun_({
            {"hall_texture.plist", "hall_texture.png"},
            {"common_texture.plist", "common_texture.png"},
            {"lang_texture.plist", "lang_texture.png"},
            {"country_texture.plist", "country_texture.png"},
        },
        function()
